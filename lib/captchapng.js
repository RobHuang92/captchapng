...
module.exports = function captchapng(width, height, dispNumber) {
  var bgColor = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : '#FFF';
  var fgColor = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : [];
  ...
  var png = new _nodePnglib2.default(this.width, this.height, this.depth, bgColor);

  if (fgColor && !fgColor.length) {
    for (var i = 0; i < 7; i++) {
      fgColor.push(randomColor());
    }
  }

  fgColor.map(function (item) {
    png.color(item);
  });
